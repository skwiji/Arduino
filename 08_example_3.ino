...
float last_reading; // unit: mm
unsigned long last_sampling_time; // unit: ms
float scale; // used for pulse duration to distance conversion
int pwmValue; // pwmValue = 0~254 를 밝기단계 100에 맞춰서
...
// initialize last_reading. =직전에 읽었던 값. reading결과가 0이면 이값을 사용한다.
  last_reading=0;
}
...
// output the read value to the serial port
  ...
  Serial.println("Max:400");
    ...
    //pwm v 계산한다.
    pwmValue = 254 *abs(200 - dist_raw)/100;
