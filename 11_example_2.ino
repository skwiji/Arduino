...
#define _DUTY_NEU 1486 // servo neutral position (90 degree)
#define _DUTY_MAX 2399 // servo full counterclockwise position (180 degree)
#define _DIST_ALPHA 0.95 // EMA weight of new sample (range: 0 to 1). Setting this value to 1 effectively disables EMA filter.

// global variables
float timeout; // unit: us
float dist_min, dist_max, dist_raw, dist_prev, dist_ema, alpha; // unit: mm
float read_raw,read_prev;
...
// get a distance reading from the USS
  read_raw = USS_measure(PIN_TRIG,PIN_ECHO);

  dist_raw=read_raw*scale;
...
_DUTY_degree = _DUTY_MIN+((dist_ema-180)/180)*(_DUTY_MAX-_DUTY_MIN);
  if (dist_ema <= 180 ){
    myservo.writeMicroseconds(_DUTY_MIN);
  }
  else if (dist_ema > 180 && dist_ema < 360){
    myservo.writeMicroseconds(_DUTY_degree);
  }
  else{
...
float USS_measure(int TRIG, int ECHO)
{
  ...
  reading = pulseIn(ECHO, HIGH, timeout);
  if(reading==0){ //측정실패
    reading = read_prev;
